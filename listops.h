...
static inline int list_length(NODE *list) {
    int i = 0;
    while (list) {
        i++;
...
static inline NODE* list_join(NODE *a, NODE *b) {
    NODE *head = a;
...
static inline NODE* list_reverseInPlace(NODE *list, NODE *tail) {
    if (!list) return tail;
    NODE* next = asNODE(list->addr);
    list->addr = asVALUE(tail);
    return list_reverseInPlace(next, list);
}

static inline void list_push(void *val, NODE **list) {
    *list = newNODE(val,(*list));
}

static inline VALUE* list_pop(NODE **list) {
...
static inline VALUE* list_peak(NODE **list) {
    failNIL(list,"NIL cannot be peaked");
    return (*list)->data;
}

static inline NODE* list_reverse(NODE *list) {
    return list_reverseInPlace(list,NIL);
}
